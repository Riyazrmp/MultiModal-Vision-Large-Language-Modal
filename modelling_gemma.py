...
class GemmaAttention(nn.Module):
     ...
     def forward(self, hidden_states: torch.Tensor,
                 ...
                 kv_cache: Optional[KVCache] = None)-> Tuple[torch.Tensor,Optional[torch.Tensor],Optional[Tuple[torch.Tensor]]]:
...
class GemmaModel(nn.Module):
    ...
    def forward(self,attention_mask: Optional[torch.Tensor]= None,
        ...
        for decoderLayer in self.layers:
             hidden_states = decoderLayer(hidden_states,attention_mask= attention_mask,position_ids=position_ids,kv_cache=kv_cache )
...
class PaliGemmaForConditionalGeneration(nn.Module):
    def __init__(self, config: PaliGemmaConfig):
        ...
        self.multi_head_projector = PaliGemmaMultiModelProjector(config)
        self.vocab_size = config.vocab_size
    ...
    def _merge_input_ids_with_image_features(self, image_features:torch.Tensor, input_embeds: torch.Tensor, input_ids: torch.Tensor, attention_mask: torch.Tensor,kv_cache):
    ...
    def forward(self,input_ids:torch.LongTensor = None,
                ...
                image_features = self.multi_head_projector(selected_image_features)
